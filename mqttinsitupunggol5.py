...
class soundplayer:
    ...
    def playtesttone(self):
        print('playing testtone')
        os.system(f'aplay -D {self.defaultaplaydevice} {cwd}/4channel_fade.wav')
    ...
    def playbirdprior(self):
        ...
        print(self.maskerpath + 'bird_prior.wav')
        print('now playing fixed masker {} with gain: {} as DOA {}'.format('bird_prior', bird_priorgain*compGain, self.currentdoa))
        print(f'\t\t{self.maskerpath} || {self.playbackmasker} || bird_prior.shape = {bird_prior.shape} || fs = {fs} || bird_priorgain = {bird_priorgain} || compGain = {compGain} || self.playbacksr = {self.playbacksr}')
        sf.write(self.playbackmasker, bird_prior*bird_priorgain*compGain, self.playbacksr)
    ...
    def playmasker(self):
        ...
        try:
            ...
            compGain = math.pow(10,self.insitucompensate(numofspeakers,optimaldistance)/20)
            print('Compensated gain: {} dB'.format(20*math.log10(compGain)))
            print(f'\t\t{self.maskerpath} || {self.playbackmasker} || masker.shape = {data1.shape} || fs = {fs1} || bird_priorgain = {self.weightedgain1} || compGain = {compGain} || self.playbacksr = {self.playbacksr}')
            sf.write(self.playbackmasker, data1*compGain, self.playbacksr)
            print(f'now executing {self.playbackcommand}')
            os.system(self.playbackcommand)
            try:
                amssClient.publish(topic=amssTOPIC, payload = (str(predictionsdict)), QoS=mqtt.QoS.AT_MOST_ONCE)
